...
def plotter(data, field):
    ...
    gp.plot((np.array([convert(datum) for datum in data]),
             np.array([float(x[field]) for x in data]),
             {'with': 'lines'}),
            terminal='dumb 200, 50',
            unset='grid',
            ascii=True,
            output="temp.txt",
            xlabel="Hour (24)",
...
if __name__ == "__main__":
    ...
    option_parser.add_option(
        ...
        default=1,
        type="int",
        help='How many days back you want to run on the data.')
    option_parser.add_option(
        '-p',
        '--plot',
        action='store_true',
        dest='plot',
        help='Plot the upload and download speed over the course of the day.')
