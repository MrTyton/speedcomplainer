...
class PingTest():
    ...
    def doPingTest(self):
        response = os.system(
            "ping -c %s -W %s -w %s 8.8.8.8 > /dev/null 2>&1" %
    ...
    def logPingResults(self, pingResults):
        self.logger.log([pingResults['date'].strftime('%Y-%m-%d'),
                         pingResults['date'].strftime('%H:%M:%S'),
...
class SpeedTest():
    ...
    def logSpeedTestResults(self, speedTestResults):
        self.logger.log([speedTestResults['date'].strftime('%Y-%m-%d'),
                         speedTestResults['date'].strftime('%H:%M:%S'),
    ...
    def tweetResults(self, speedTestResults):
        ...
        for (threshold, messages) in thresholdMessages.items():
            ...
            if speedTestResults['downloadResult'] < threshold:
                message = messages[random.randint(0,
                                                  len(messages) - 1)].replace('{tweetTo}',
                                                                              self.config['tweetTo']).replace('{internetSpeed}',
                                                                                                              self.config['internetSpeed']).replace('{downloadResult}',
                                                                                                                                                    str(speedTestResults['downloadResult']))

        if message:
            api = twitter.Api(
                consumer_key=self.config['twitter']['twitterConsumerKey'],
